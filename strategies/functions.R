...
bpv <- function(ds, crs) { # name of data set, output from gs.cross.eval()
  ...
  stopifnot(sum(mds$genotypes$`Mar/Ind` == yld.eff$marker[2:(nm+1)])==(nrow(yld.eff)-1))
  mds$genotypes$allele1 <-  yld.eff$allele[2:(nm+1)] # first element is the mean
  mds$genotypes$allele2 <-  yld.eff$cmpallele[2:(nm+1)]
  mds$genotypes$effect <- abs(yld.eff$effect[2:(nm+1)])
