...
# Cross selected parental lines                        # CR-IL: BPV

st.get.simpop("Psel","Psel")
gs.set.effects(eff=yld.eff,data.set="Psel")
gs.cross.eval.mu(data.set="Psel")
crs.Psel  <- gs.cross.info(data.set="Psel")

crs.Psel <- bpv("Psel", crs.Psel)
...
crs.Psel  <- gs.cross.info(data.set="F1",)
crs.Psel <- bpv("F1", crs.Psel)
