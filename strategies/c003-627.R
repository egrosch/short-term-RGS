...
st.input.dir  <- "input"
st.output.dir <- "output627"
dir.create(st.output.dir)
st.set.info.level (-2)
gs.set.num.threads(2)

dbfile <- "data/c003_627.sqldb" # database
...
crs.Psel  <- gs.cross.info(data.set="Psel",sortby ="es")

crs <- sel.set(ncp=1, nct=72, crs.list=crs.Psel)
...
crs.Psel  <- gs.cross.info(data.set="F1",sortby ="es")      # CR-F1: UC-OCS
...
C <- 2; {
    ...
    crs.Psel  <- gs.cross.info(data.set="SYN",sortby ="es")      # CR-C2: UC-OCS
...
C <- 3; {
    ...
    gs.cross.eval.va(pop.type="CRS", data.set="SYN")
    gs.cross.eval.es(alpha=0.2,data.set="SYN")
    crs.Psel  <- gs.cross.info(data.set="SYN",sortby ="es")      # CR-C3: UC-OCS
