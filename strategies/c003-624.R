...
st.input.dir  <- "input"
st.output.dir <- "output624"
dir.create(st.output.dir)
st.set.info.level (-2)
gs.set.num.threads(2)

dbfile <- "data/c003_624.sqldb" # database
...
C <- 2; {
    ...
    crs  <- gs.cross.info(data.set="SYN",sortby ="es")      # CR-C2: MSD
    d.sorted <- data.frame(OTU1     =crs$P1No,
                           OTU2     =crs$P2No,
...
C <- 3; {
    ...
    gs.cross.eval.va(pop.type="CRS", data.set="SYN")
    gs.cross.eval.es(alpha=0.2,data.set="SYN")
    crs  <- gs.cross.info(data.set="SYN",sortby ="es")      # CR-C3: MSD
    d.sorted <- data.frame(OTU1     =crs$P1No,
                           OTU2     =crs$P2No,
                           Measure  =crs$es,stringsAsFactors=FALSE)
    crs     <- sel.crs (d.sorted,ncp=1,nct=36)  
